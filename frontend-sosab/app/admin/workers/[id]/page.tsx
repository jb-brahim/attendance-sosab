...
/* ── Heatmap ── */
function buildSlots(history: DayRecord[]) {
  ...
  const slots: { date: string; status: string | null }[] = [];
  const cur = new Date(start);
  const todayStr = today.toISOString().split('T')[0];
  while (cur.toISOString().split('T')[0] <= todayStr) {
    const key = cur.toISOString().split('T')[0];
    const inRange = cur >= new Date(new Date().setDate(new Date().getDate() - 89));
    slots.push({ date: key, status: inRange ? (map.get(key) ?? null) : null });
...
function cellColor(status: string | null) {
  if (status === null)      return 'bg-muted/40';
...
function HeatCell({ date, status }: { date: string; status: string | null }) {
  const [tip, setTip] = useState(false);
  return (
    <div className="relative" onMouseEnter={() => setTip(true)} onMouseLeave={() => setTip(false)}>
      <div className={`w-4 h-4 rounded-sm cursor-default ${cellColor(status)}`} />
      {tip && status !== null && (
        <div className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 whitespace-nowrap bg-popover border border-border text-[10px] font-medium text-foreground px-2 py-1 rounded-lg shadow-lg pointer-events-none">
          {date} · <span className="capitalize">{status}</span>
        </div>
      )}
    </div>
  );
...
export default function AdminWorkerDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const wRes = await workerAPI.getAll();
        const found = (wRes.data.data || []).find((w: any) => w._id === id);
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-card border border-border/60 rounded-2xl p-6 flex flex-col sm:flex-row sm:items-center gap-5 shadow-sm">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-1 mt-3 text-xs text-muted-foreground">
            ...
            <span className="flex items-center gap-1.5">
              ...
              {t('common.joined')} {new Date(worker.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}
            </span>
          </div>
        </div>
        ...
        <button
          ...
          className="flex-shrink-0 flex items-center gap-1.5 text-xs font-medium text-muted-foreground hover:text-foreground border border-border/60 px-3 py-1.5 rounded-lg transition cursor-pointer"
        >
          <Edit className="w-3.5 h-3.5" /> {t('workerDetail.editProfile')}
        </button>
      </div>
