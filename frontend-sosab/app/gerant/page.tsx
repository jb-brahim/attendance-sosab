...
export default function GerantOverviewPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-amber-500/90 mb-1">{t('dashboard.siteManager')}</p>
          <h2 className="text-2xl font-semibold text-foreground tracking-tight">{t('dashboard.dailyAttendance')}</h2>
        </div>
        <button
          onClick={() => load(day, true)}
          disabled={refreshing}
          className="flex items-center gap-2 px-3.5 py-2 bg-secondary hover:bg-secondary/85 border border-border rounded-lg text-foreground text-xs font-medium transition cursor-pointer disabled:opacity-50"
        >
      ...
      <div className="glass-card rounded-xl overflow-hidden">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-muted-foreground text-xs font-medium">Loading roster data...</p>
          </div>
        ) : workers.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-2">
            <HardHat className="w-7 h-7 text-slate-700" />
            <p className="text-muted-foreground text-xs font-medium">No personnel registered in the system.</p>
          </div>
        ) : (
